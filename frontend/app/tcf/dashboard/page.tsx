...
export default function DashboardPage() {
  ...
  return (
    <TcfAppShell title="TCF Dashboard" subtitle="Your progress at a glance">
              ...
              <Card className="border-slate-200 shadow-sm">
                ...
                <CardContent className="space-y-3">
                  ...
                  {summary.writing.average_score < 6 && summary.writing.recent_submissions.length > 0 && (
                    <div className="flex gap-3 rounded-xl bg-amber-50 p-3">
                      <AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0 text-amber-500" />
                      <p className="text-xs text-amber-700">Writing score under 6/10 - use Learn to practice sentence structure.</p>
                    </div>
                  )}
                  {summary.learning.recent_sessions.length === 0 && (
                    <div className="flex gap-3 rounded-xl bg-emerald-50 p-3">
                      ...
                      <p className="text-xs text-emerald-700">You have not tried Learn yet - paste any French text to generate exercises.</p>
                    </div>
                  )}
                  {overview.overall >= 75 && (
                    <div className="flex gap-3 rounded-xl bg-emerald-50 p-3">
